...
export function EcosystemStatusBar() {
  ...
  const healthLabel = !vitals ? 'Connecting' : organismAlive ? 'Optimal' : 'Degraded'
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-20 hidden items-center justify-between px-8 py-2.5 md:flex pointer-events-none">
      <div className="flex items-center gap-2">
        <span className={`inline-block h-1.5 w-1.5 rounded-full ${organismAlive ? 'bg-secondary' : 'bg-on-surface-muted/30'}`} />
